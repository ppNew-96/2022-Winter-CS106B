 ...
 */
double weightOnBackOfHelper(int row, int col, Map<GridLocation, double> &memoization)
{
    if (row == 0)
        return 0;
    else
    {
        GridLocation cur = {row, col};
        if (!memoization.containsKey(cur))
        {
            double curWeight = 0;
            if (col == 0 || col == row)
                curWeight = 0.5 * (weightOnBackOfHelper(row - 1, 0, memoization)) + 80;
            else
                curWeight = 0.5 * (weightOnBackOfHelper(row - 1, col - 1, memoization) + weightOnBackOfHelper(row - 1, col, memoization)) + 160;
            memoization.put(cur, curWeight);
        }
...
double weightOnBackOf(int row, int col, int pyramidHeight)
{
    ...
    if (row < 0 || col < 0 || pyramidHeight < 0)
        error("row, col or height is negtive.");
    else if (row > pyramidHeight - 1)
        error("pyramidHeight is not enough.");
    else if (row < col)
        error("row is less than col.");

    Map<GridLocation, double> memoization;

    return weightOnBackOfHelper(row, col, memoization);
}
...
STUDENT_TEST("Check Person G, H, L, M.")
{
