...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     */
    if(kGramLength<=0)
        error("Illegal kGramLength, which must be positive.");
    int len = str.size();
    if(len<kGramLength)
        return {};
    Map<string,double> res={};
    for(int i=0;i<=len-kGramLength;i++){
       string curSubStr=str.substr(i,kGramLength);
       if(res.containsKey(curSubStr))
       {
           res[curSubStr]++;
       }
       else{
...
Map<string, double> normalize(const Map<string, double>& input) {
     ...
     */
    if(input.isEmpty())
        error("No elements int the map.");

    double squareSum=0;
    for(string key:input.keys())
    {
        squareSum+=pow(input[key],2);
    }
    if(squareSum==0)
        error("Frequencies are all zero.");
    ...
    for(string key:input.keys())
    {
        res.put(key,input[key]/squareRoot);
    }
    return res;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    else
    {
        ...
        Map<string,double> res=source;
        PriorityQueue<string> pq;
        for(string key:source)
        {
            pq.enqueue(key,source[key]);
        }
        int abandon = index-numToKeep;
        while(abandon!=0)
        {
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
     ...
     * this function.
     */
    double cosineSimilar=0;
    for(string key:lhs)
    {
       if(rhs.containsKey(key))
           cosineSimilar+=lhs[key]*rhs[key];
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    {
        double curSimilar=cosineSimilarityOf(textProfile,elem.profile);
        if(curSimilar>cosSimilar)
        {
            cosSimilar=curSimilar;
            curLanguage=elem.name;
        }
    }
    return curLanguage;
}
