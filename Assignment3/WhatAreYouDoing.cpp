 ...
 */
void allEmphasesOfHelp(Vector<string> &tokens, Set<string> &res, string curSentence, int index)
{
    ...
    {
        string curToken = tokens[index];
        if (!isalpha(curToken[0]))
        {
            curSentence += curToken;
            allEmphasesOfHelp(tokens, res, curSentence, index + 1);
        }
        else
        {
            string upperToken = toUpperCase(curToken);
            string temp = curSentence + upperToken;
            allEmphasesOfHelp(tokens, res, temp, index + 1);

            temp = curSentence + curToken;
            allEmphasesOfHelp(tokens, res, temp, index + 1);
        }
    }
}
Set<string> allEmphasesOf(const string &sentence)
{
    ...
    Vector<string> tokens = tokenize(lowerSentence);
    Set<string> res = {};
    string curSentence = "";
    int index = 0;
    allEmphasesOfHelp(tokens, res, curSentence, index);
    return res;
}
