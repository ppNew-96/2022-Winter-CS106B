                              ...
                              int weight, Set<Pair> &maxWeightMatching, int &maxWeight)
{
    if (possiblePairs.size() == 0)
    {
        if (weight > maxWeight)
        {
            // record matching set with maxWeight.
    ...
    {
        Pair newPair = possiblePairs.firstKey();

        // branch1: final result won't contain this newPair.
        Map<Pair, int> withoutRemain = possiblePairs;
        withoutRemain.remove(newPair);
        maximumWeightMatchingRec(withoutRemain, matching, weight, maxWeightMatching, maxWeight);

        // branch2: final result will contain this newPair.
        Map<Pair, int> withRemain = possiblePairs;
        for (const Pair &elem : possiblePairs)
        {
            if (elem.first() == newPair.first() || elem.second() == newPair.second() || elem.first() == newPair.second() || elem.second() == newPair.first())
            {
