...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Queue<GridLocation> waterSource;
    Grid<bool> res;
    res.resize(terrain.numRows(),terrain.numCols());

    for(GridLocation elem:sources){
        if(terrain.get(elem)<=height)
        {
            waterSource.enqueue(elem);
            res[elem]=true;
        }
    }

    while(!waterSource.isEmpty()){
       GridLocation cur=waterSource.dequeue();
       if(terrain.inBounds(cur.row-1,cur.col)&&res[cur.row-1][cur.col]!=true&&terrain[cur.row-1][cur.col]<=height)
       {
           waterSource.enqueue({cur.row-1,cur.col});
           res[cur.row-1][cur.col]=true;
       }
       if(terrain.inBounds(cur.row+1,cur.col)&&res[cur.row+1][cur.col]!=true&&terrain[cur.row+1][cur.col]<=height)
       {
           waterSource.enqueue({cur.row+1,cur.col});
       ...
       if(terrain.inBounds(cur.row,cur.col-1)&&res[cur.row][cur.col-1]!=true&&terrain[cur.row][cur.col-1]<=height)
       {
           waterSource.enqueue({cur.row,cur.col-1});
           res[cur.row][cur.col-1]=true;
       }
       if(terrain.inBounds(cur.row,cur.col+1)&&res[cur.row][cur.col+1]!=true&&terrain[cur.row][cur.col+1]<=height)
       {
           waterSource.enqueue({cur.row,cur.col+1});
