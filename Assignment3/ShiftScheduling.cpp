...
int sumValueOf(const Set<Shift> &shifts)
{
    int sumValue = 0;
    for (const Shift &elem : shifts)
    {
        sumValue += valueOf(elem);
    }
    return sumValue;
}

Set<Shift> highestHelper(const Set<Shift> &shifts, const Set<Shift> &res, int hourRemain)
{
    if (hourRemain < 0)
    {
